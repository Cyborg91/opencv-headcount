...
def draw_detections(img, rects, fgmask, thickness=2):
    ...
    for x, y, w, h in rects:
        ...
        if w * h > PERSON_SIZE:
            ...
            for id, person in enumerate(persons):
                if active and person.dist(x_0, y_0) <= w / 2 and person.dist(x_0, y_0) <= h / 2:
                    new = False
                    if person.x < midline < x_0:
                        outcount += 1
                    elif person.x > midline > x_0:
                        incount += 1
                    person.x = x_0
                    person.y = y_0
                    break
    ...
    cv2.putText(img, 'In:{},Out:{}'.format(incount, outcount), (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1,
                cv2.LINE_AA)


url = 'rtsp://192.168.0.9:554/user=admin&password=900009&channel=1&stream=0.sdp?'
rurl = 'rtsp://103.21.234.247:554/user=admin&password=900009&channel=1&stream=0.sdp?'
file = 'test.mp4'
...
while (1):
    ...
    (_, allContours, _) = cv2.findContours(fgmask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    cv2.imshow('frame', fgmask)#cv2.bitwise_and(frame,frame,mask=fgmask))
    #out.write(frame)
